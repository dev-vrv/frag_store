...
const CyberHero = React.forwardRef<HTMLElement, CyberHeroProps>(
  (
    ...
    <section
      ref={ref}
      className={cn(
        "relative isolate overflow-hidden bg-surface px-4 py-20 text-zinc-50 sm:px-6 lg:px-8",
        className,
      )}
      {...props}
    >
      <div className="absolute inset-0 -z-20 bg-[radial-gradient(circle_at_20%_20%,rgba(248,113,113,0.18),transparent_32%),radial-gradient(circle_at_80%_10%,rgba(217,70,239,0.15),transparent_28%),linear-gradient(180deg,rgba(var(--theme-surface-rgb),1),rgba(var(--theme-surface-rgb),1))]" />
      ...
      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_bottom,transparent_50%,rgba(var(--theme-contrast-rgb),0.025)_51%,transparent_52%)] bg-[length:100%_7px] opacity-40" />
      ...
      <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="relative min-h-[320px] lg:min-h-[520px]">
          {media ?? (
            <div className="absolute inset-0 rounded-md border border-red-300/20 bg-[radial-gradient(circle_at_50%_20%,rgba(248,113,113,0.2),transparent_34%),linear-gradient(135deg,rgba(39,39,42,0.6),rgba(var(--theme-surface-rgb),0.95))] shadow-[0_0_70px_rgba(248,113,113,0.18)]">
              <div className="absolute inset-4 rounded-md border border-white/10 bg-surface/35 backdrop-blur-sm" />
              ...
              <div className="absolute right-12 bottom-12 left-12 grid grid-cols-3 gap-3">
                {["GPU", "SYNC", "240HZ"].map((item) => (
                  <div
                    key={item}
                    className="rounded-md border border-red-300/20 bg-surface/45 p-3 text-center font-mono text-xs tracking-[0.18em] text-red-100"
                  >
                    {item}
                  </div>
                ))}
